...
class JTNNVAE(nn.Module):
    ...
    def __init__(self, vocab, hidden_size=450, latent_size=56, depthT=20, depthG=3):
        ...
        self.jtnn = JTNNEncoder(hidden_size, depthT, nn.Embedding(vocab.size(), hidden_size)).to(self.device)
        self.decoder = JTNNDecoder(vocab, hidden_size, latent_size, nn.Embedding(vocab.size(), hidden_size)).to(self.device)

        self.jtmpn = JTMPN(hidden_size, depthG).to(self.device)
        self.mpn = MPN(hidden_size, depthG).to(self.device)

        self.A_assm = nn.Linear(latent_size, hidden_size, bias=False).to(self.device)
        self.assm_loss = nn.CrossEntropyLoss(size_average=False).to(self.device)

        self.T_mean = nn.Linear(hidden_size, latent_size).to(self.device)
        self.T_var = nn.Linear(hidden_size, latent_size).to(self.device)
        self.G_mean = nn.Linear(hidden_size, latent_size).to(self.device)
        self.G_var = nn.Linear(hidden_size, latent_size).to(self.device)
    ...
    def encode_from_smiles(self, smiles_list):
        ...
        return torch.cat([tree_vecs.data, mol_vecs.data], dim=-1)
    ...
    def encode_latent(self, smiles_list):
        ...
        return torch.cat([tree_mean, mol_mean], dim=1).data, torch.cat([tree_var, mol_var], dim=1).data
